...
public class DevLib
{
    ...
    private string GetSQL_Details(int Index = -1)
    {
        string strReturn = string.Empty;
        try
        {
            //EventLog("GetSQL_Details");
            string StrCompID = CurrentData.get().CompanyId.ToString();
            //EventLog("CompId:" + StrCompID);
            string CompCode = DatabaseWrapper.GetCompanyCode(Convert.ToInt32(StrCompID));
            //EventLog("CompCode:" + CompCode);
            string[] SqlCon = DatabaseWrapper.GetDatabaseDetails();
            //EventLog("SqlCon Len:" + Convert.ToString(SqlCon.Length));
            string SQLServer = SqlCon[0];
            //EventLog("SqlCon[0]:" + SqlCon[0]);
            string SQLUser = SqlCon[1];
            //EventLog("SqlCon[1]:" + SqlCon[1]);
            string SQLPW = SqlCon[2];
            //EventLog("SqlCon[2]:" + SqlCon[2]);

            if (Index == -1)
            {
                strReturn = SQLServer + ";" + SQLUser + ";" + SQLPW;
            }
            else if (Index == 0)
            {
                strReturn = SQLServer;
        ...
        {
            ErrLog(ex, "DevLib.GetSQL_Details()");
        }
        return strReturn;
    }
    ...
    public void RegKeyCreate(string keyName, string keyValue)
    {
        try
        {
            ...
            {
                RegistryKey key;
                key = Registry.Users.CreateSubKey(".Default\\Software\\Focus8"
                    );
            ...
            {
                //MsgBox(2, "Load as an Administrator");
                EventLog("RegKeyCreate (Run as an Administrator) : keyName=" + keyName + "; keyValue=" + keyValue);
            }

        }
        catch (Exception ex)
        {
            ErrLog(ex, "Triggers.RegKeyCreate()");
        }
    }


    public bool DBUpdate_Common()
    {
        bool rtnFlag = false;
        try
        {
            DBScript dbScript = new DBScript();
            rtnFlag = dbScript.DB_Update();
        }
        catch (Exception ex)
        {
            ErrLog(ex, "DevLib.DBUpdate_Common()");
            rtnFlag = false;
        }
        return rtnFlag;
    }
    ...
    /// <param name="EntityType">P : Procedure
    /// V : View
    /// F : Function
    /// U : User Table
    /// S : System Table
    ...
    /// XP : Extended Stored Procedure.</param>
    /// <returns></returns>
    public bool ExistsDBEntity(string EntityName, string EntityType)
    {
        bool rtnFlag = false;
        try
        {
            string SQL = @"IF (EXISTS (SELECT * FROM sysobjects WHERE type = '" + EntityType + "'And name = '" + EntityName + "')) " +
                            @"BEGIN
                                    Select 1 [Exists]
                            END
                            ELSE
        ...
        {
            ErrLog(ex, "Triggers.ExistsDBEntity(" + EntityName + "," + EntityType + ")");
            rtnFlag = false;
        }
        return rtnFlag;
    }
    ...
    */
    ...
    public string SessionId = Convert.ToString(System.Windows.Application.Current.Properties["SessionId"]);
    ...
    public string BaseUrl()
    {
        ...
        try
        {
            string[] url = Convert.ToString(System.Windows.Application.Current.Properties["DataManagement"]).Split('/');
            if (url.Length > 2)
            {
                rtnResult = url[0] + "/" + url[1] + "/" + url[2] + "/Focus8API/";
            }
        }
        catch (Exception ex)
        {
            ErrLog(ex, "DevLib.BaseUrl()");
        }
        return rtnResult;
    }
    ...
    public string GetNameOfTag(int iMasterTypeId)
    {
        string TableName = "";
        try
        {
    ...
    */
    ...
    {
        EventLog("DefaultFormSearchByTags Function called ");
        DataTable dt_DF = new DataTable();
        dt_DF.Columns.Add("RowNo");
        dt_DF.Columns.Add("Param");
        dt_DF.Columns.Add("Value");
        EventLog("Temp Table Created");
        try
        {
            ...
            //if (Tag.Length == 1)
            {
                ...
                for (int i = 0; i < Tag.Length; i++)
                {
                    EventLog("ComboBox Loop - " + Convert.ToString(i));
                    ComboBox cbx = new ComboBox();
                    EventLog("ComboBox initiated");
                    cbx.Location = new System.Drawing.Point(160, (35 * i) + 82);
                    cbx.Width = 200;
                    cbx.Name = "cbxTag" + Convert.ToString(i);
                    EventLog("cbx.Name : " + cbx.Name);
                    cbx.Tag = Tag[i].TagId;
                    EventLog("cbx.Tag : " + cbx.Tag);
                    UIDesign(cbx);
                    f.Controls.Add(cbx);
                    EventLog("Control Added");

                    Label lbl = new Label();
                    EventLog("label initiated");
                    lbl.Location = new System.Drawing.Point(50, (35 * i) + 82);
                    lbl.Width = 200;
                    lbl.Name = "lbl" + Convert.ToString(i);
                    EventLog("lbl.Name : " + lbl.Name);
                    if (Convert.ToString(Tag[i].TagLabel) == null)
                    {
                        lbl.Text = GetNameOfTag(Tag[i].TagId);
                    }
                ...
                {
                    EventLog("RowNo:" + Convert.ToString(dt_DF.Rows[i]["RowNo"]) + "\n\r" +
    ...
    /// <param name="DateRange">Default Value = 0, 0 = DateRange, 1 = As On Date, 2 = As On Month, 3 = As On Year, -1 = Hide Date </param>
    /// <returns>DataTable</returns>
    public DataTable DefaultFormSearchByTags(string ScreenTitle, MultiSelectTags[] multiSelect, SearchByTags[] Tag, int DateRange = 0, string ReportName = "", string ProcedureName = "", bool MinimizeOption = true)
    {
        EventLog("DefaultFormSearchByTags Initiated");
        ...
        try
        {
            EventLog("frmMultiSearch");
            frmMultiSearch f = new frmMultiSearch();
            f.Text = ScreenTitle;

            #region Date Fields
            EventLog("Set Start Date");
            f.dtpStartDate.Value = GetAccountingDate();
            EventLog(Convert.ToString(DateRange));
            if (DateRange == 0)             // DateRange
            {
                f.dtpStartDate.Enabled = true;
                f.pbxStartDate.Enabled = true;
            ...
            EventLog("Multi Selection ComboBox");
            ...
            for (int i = 0; i < multiSelect.Length; i++)
            {
                DataTable dt_Tag = new DataTable();
                #region Tab0
                EventLog("Tab" + Convert.ToString(i));
                if (i == 0)
                {
            ...
            #endregion
            EventLog("Single Selection ComboBox");
            #region Single Selection ComboBox
            ...
            if (dialogResult == DialogResult.OK)
            {
                EventLog("DialogResult OK");
                string iStartDate = Convert.ToString(FocusDateToInt(f.dtpStartDate.Value.ToString("yyyy/MM/dd")));
                string iEndDate = Convert.ToString(FocusDateToInt(f.dtpEndDate.Value.ToString("yyyy/MM/dd")));

                EventLog("DateRange = " + Convert.ToString(DateRange));
                if (DateRange == 2)        // As On Month
                {
                    dt_DF.Rows.Add("1", "iStartDate", Convert.ToString(FocusDateToInt(GetFirstDayOfMonth(f.dtpEndDate.Value).ToString("yyyy/MM/dd"))));
                    dt_DF.Rows.Add("2", "iEndDate", Convert.ToString(FocusDateToInt(GetLastDayOfMonth(f.dtpEndDate.Value).ToString("yyyy/MM/dd"))));
                ...
                for (int i = 0; i < multiSelect.Length; i++)
                {
                    EventLog("MultiSelectTag" + Convert.ToString(multiSelect[i].TagId));
                    if (i == 0)
                    {
                ...
                for (int i = 0; i < Tag.Length; i++)
                {
                    EventLog("cbxTag" + Convert.ToString(i));
    ...
    public DataTable MultiTag_DataTable(int TagId, int ItemType)
    {
    ...
    public void chkGroup_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
    ...
    /// /// <param name="Open">Set true to open a file after write</param>
    /// <returns></returns>
    public bool WriteLog(string LogString, string LogFileName, bool Open = false)
    {
        string exePath = (Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().GetName().CodeBase) + "\\Log").Replace("file:\\", "");
        ...
        using (StreamWriter w = File.AppendText(curFile))
        {
            w.WriteLine(Convert.ToString(LogString));
        }

        using (StreamReader r = File.OpenText(curFile))
        {
            DumpLog(r);
        }
        if (Open)
    ...
    public bool EventLog(string Comment = "", [CallerLineNumber] int lineNumber = 0, [CallerMemberName] string Member = null)
    {
        try
        {
            if (GetValueFromRegistry("Debug") == "1")
            {
                ...
                Comment = Convert.ToString(Comment) != "" ? " >> " + Convert.ToString(Comment) : "";
